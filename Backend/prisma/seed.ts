...
async function main() {
    ...
    // --- Auth users: admin (scoped to the org above) + a demo customer ---
    const admin = await prisma.user.create({
        data: {
            ...
            role: "ADMIN",
            status: "ACTIVE",
            emailVerified: true, // admins skip verification entirely
            organizationId: org.id,
        },
    });
    ...
    const customer = await prisma.user.create({
        data: {
            ...
            role: "CUSTOMER",
            status: "ACTIVE",
            emailVerified: true, // seeded for convenience; real signups still go through the OTP flow
            // organizationId intentionally left null — customers aren't org-scoped
        },
    });
    ...
    console.log("Customer login:", customer.email, "/ Customer@12345");
}
